-- ============================================================
-- all.sql — 依 sql/MIGRATION_ORDER.md 順序合併的完整遷移檔
-- 由 sql/ 目錄下各檔案依檔名順序串接而成
-- 不含：cron_*（定時任務）、debug_*（除錯用）
-- fix_*／security_* 開頭的檔案（未被 MIGRATION_ORDER.md 編號）統一放在最後
-- ============================================================

-- 6_7_recurring_batch2.sql / 6_8_recurring_batch3.sql 會呼叫 cron.schedule(...)，
-- 需要 pg_cron 擴充功能先啟用。這裡只啟用擴充功能本身，
-- 不包含 cron_*.sql 裡實際排程的定時任務內容。
...
-- students
DROP POLICY IF EXISTS "anon can select students" ON students;
...
-- student_classes
DROP POLICY IF EXISTS "anon can select student_classes" ON student_classes;
CREATE POLICY "anon can select student_classes"     ON student_classes FOR SELECT TO anon          USING (true);
DROP POLICY IF EXISTS "auth full access on student_classes" ON student_classes;
...
DROP POLICY IF EXISTS "anon can select esf" ON event_session_fields;
CREATE POLICY "anon can select esf"                 ON event_session_fields FOR SELECT TO anon          USING (true);
DROP POLICY IF EXISTS "auth full access on esf" ON event_session_fields;
...
DROP POLICY IF EXISTS "auth full access on registrations" ON registrations;
...
DROP POLICY IF EXISTS "auth full access on changes" ON registration_changes;
...
DROP POLICY IF EXISTS "auth full access on rsc" ON registration_session_checkins;
...
-- audit_log
DROP POLICY IF EXISTS "auth full access on audit_log" ON audit_log;
...
DROP POLICY IF EXISTS "auth full access on event_donors" ON event_donors;
...
DROP POLICY IF EXISTS "auth full access on car_assignments" ON car_assignments;
...
DROP POLICY IF EXISTS "auth full access on car_members" ON car_members;
...
DROP POLICY IF EXISTS "auth full access on car_leaders" ON car_leaders;
...
DROP POLICY IF EXISTS "anon can select head_leader" ON head_leader;
CREATE POLICY "anon can select head_leader"         ON head_leader     FOR SELECT TO anon USING (true);
DROP POLICY IF EXISTS "auth full access on head_leader" ON head_leader;
...
-- relationship
DROP POLICY IF EXISTS "auth full access on rel_groups" ON relationship_groups;
CREATE POLICY "auth full access on rel_groups"      ON relationship_groups  FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "auth full access on rel_members" ON relationship_members;
...
-- volunteer
DROP POLICY IF EXISTS "auth full access on vol_profiles" ON volunteer_profiles;
CREATE POLICY "auth full access on vol_profiles"    ON volunteer_profiles     FOR ALL TO authenticated USING (true) WITH CHECK (true);
DROP POLICY IF EXISTS "auth full access on vol_access" ON volunteer_event_access;
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：1_4_volunteer_access_setup.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
-- 義工存取設定 — 建立所需資料表
...
DROP POLICY IF EXISTS "volunteer_profiles: authenticated 可讀寫" ON volunteer_profiles;
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：1_6_relationship_setup.sql
-- ════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════
-- 批次 C：關係連結系統
...
DROP POLICY IF EXISTS "authenticated can manage relationship_groups" ON relationship_groups;
...
DROP POLICY IF EXISTS "authenticated can manage relationship_members" ON relationship_members;
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：2_1_registration_tracking_setup.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：2_2_cancel_registration_setup.sql
-- ════════════════════════════════════════════════════════════
-- ══════════════════════════════════════════════
-- 取消報名功能 — 補 DELETE 權限
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：2_3_guest_registration_setup.sql
-- ════════════════════════════════════════════════════════════
-- ══════════════════════════════════════════════
-- 訪客報名功能 — 資料庫調整
-- 在 Supabase Dashboard → SQL Editor 執行一次即可
...
DROP POLICY IF EXISTS "anon can read head_leader" ON head_leader;
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：3_1_car_arrangement_setup.sql
-- ════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════
-- 批次 D：排車系統
...
DROP POLICY IF EXISTS "authenticated can manage car_assignments" ON car_assignments;
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：3_2_small_car_leader_setup.sql
-- ════════════════════════════════════════════════════════════
-- ═══════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：3_3_add_direction_to_car_assignments.sql
-- ════════════════════════════════════════════════════════════
-- 批次 4：上下山分開排車
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：3_4_fix_unique_for_direction.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：3_5_add_car_member_checkin.sql
-- ════════════════════════════════════════════════════════════
-- 上下山各自獨立報到狀態
-- 2026-05-23
--
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：4_02_add_boolean_field_type.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：4_03_add_date_field_type.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：4_05_add_host_student_id.sql
-- ════════════════════════════════════════════════════════════
-- ══════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：4_06_add_placeholder_column.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：4_07_add_activities_fields.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：4_08_add_related_links.sql
-- ════════════════════════════════════════════════════════════
-- V6: 活動相關連結
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：4_09_add_cover_image_position.sql
-- ════════════════════════════════════════════════════════════
ALTER TABLE events
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：4_11_add_volunteer_open.sql
-- ════════════════════════════════════════════════════════════
-- V5: 義工開放模式
-- 在 Supabase Dashboard → SQL Editor 執行，或 deploy 前手動跑一次
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：4_13_add_registration_source.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：5_01_add_templates_table.sql
-- ════════════════════════════════════════════════════════════
-- ══════════════════════════════════════════════════════════
...
DROP POLICY IF EXISTS "authenticated full access on event_templates" ON event_templates;
...
-- ── 插入內建模板 ─────────────────────────────────────────
-- 已停用：這裡是 5_01_add_templates_table.sql 的舊版模板內容
-- （欄位較簡略、缺少 dashboard_role/option_meta）。
-- 1_1_schema.sql 已插入內容更完整的同名模板（回山模板／精舍模板），
-- 兩者一起執行會造成 event_templates 出現重複列，故整段註解掉。
--
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：5_04_phase5_batch1_sessions.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：5_05_phase5_session_fields.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：5_06_phase5_batch1_fix_policies.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：5_07_add_is_recurring.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：5_08_create_recurring_templates.sql
-- ════════════════════════════════════════════════════════════
-- 定期活動範本表
-- 每筆範本定義一個週期性活動的基本設定
...
DROP POLICY IF EXISTS "auth users can manage recurring_templates" ON recurring_templates;
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：5_09_template_session_fields_migration.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：5_10_registration_session_checkins.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：6_1_class_normalization.sql
-- ════════════════════════════════════════════════════════════
-- ================================================================
-- 班級名稱統一化
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：6_2_update_fields_and_transport.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
-- 一次完成三件事：
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：6_3_batch_update_transport.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：6_4_update_default_templates.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：6_5_dashboard_role_migration.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：6_6_show_transport_to_public_migration.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：fix_cancel_registration_rls.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
-- 修正前台取消報名 RLS 錯誤
-- 日期：2026-07-07
...
CREATE OR REPLACE FUNCTION kiosk_submit_registration(
  p_event_id        UUID,
  p_student_id TEXT,
  p_answers         JSONB,
...
BEGIN
  ...
  VALUES (p_event_id, p_student_id, NULL, p_answers, p_terminal, p_is_driver)
...
GRANT EXECUTE ON FUNCTION kiosk_submit_registration(UUID, TEXT, JSONB, TEXT, BOOLEAN) TO anon, authenticated;
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：fix_friend_registration_rls.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
-- 修正代報親友報名 RLS 錯誤
-- 日期：2026-06-26
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：fix_get_student_by_qr_active.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：fix_recurring_fields_volunteers.sql
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：fix_rls_registrations_anon.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：fix_update_checkin_rls.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：fix_volunteer_event_access.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
...
DROP POLICY IF EXISTS "volunteer_event_access: admin 可管理" ON volunteer_event_access;
...
DROP POLICY IF EXISTS "volunteer_event_access: 義工只能讀自己" ON volunteer_event_access;
...
-- ════════════════════════════════════════════════════════════
-- 來源檔案：security_fix_car_token_security.sql
-- ════════════════════════════════════════════════════════════
-- ============================================================
